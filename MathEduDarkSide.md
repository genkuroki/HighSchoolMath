...
>の「証明」をふり返って見ると了解される.  この式は幾何学的には, 弧の長さが $0$ に近づくとき円弧と弦の長さの比が$1$ に近づくことを意味する.  このことは直銭的に明らかなように見えるが「円弧の長さ」がどのようにして定義されるのかをはっきりさせなければ, 数学的な証明とは言い難い.  また円弧の長さを例えば積分で定義したとしても, その積分を計尊するのに($*$)を用いなければならぬのでは循環論法になってしまう.
